...
def test_quote_block():
    ...
    assert produce(src) == snapshot('<blockquote> <strong>Note:</strong> This isn&#x27;t quite correct!</blockquote>')
...
def test_inline_code():
    ...
    assert produce(src) == snapshot('<p>Have you seen <code>T ** a;</code> in C before?</p>\n')


def test_inline_single_char_code():
    src = "There is `u` and `uL` in Kotlin prefixes."
    assert produce(src) == snapshot('<p>There is <code>u</code> and <code>uL</code> in Kotlin prefixes.</p>\n')


def test_comment():
    src = "Have you seen the <!-- boo & woo --> ghost?"
    assert produce(src) == snapshot('<p>Have you seen the <!-- boo & woo --> ghost?</p>\n')


def test_link():
    src = "[Homepage](https://flofriday.dev)"
    assert produce(src) == snapshot('<p><a href="https://flofriday.dev">Homepage</a></p>\n')


def test_image():
    src = "![profile picture](https://flofriday.dev/flofriday.jpg)"
    assert produce(src) == snapshot('<p><img src="https://flofriday.dev/flofriday.jpg" alt="profile picture"/></p>\n')
...
# Hello there"""
    ...
    assert ast.metadata == snapshot({'author': 'flofriday', 'title': 'An exciting test', 'date': '2024-05-16'})
...
def test_parenthesis_not_link():
    ...
    assert produce(src) == snapshot('<p>Welcome today (13.may) at this unit test.</p>\n')
...
def test_missing_closing_link_formatted():
    src = "I like [examples](https://**example.com**."
    assert produce(src) == snapshot('<p>I like [examples](https://<strong>example.com</strong>.</p>\n')
